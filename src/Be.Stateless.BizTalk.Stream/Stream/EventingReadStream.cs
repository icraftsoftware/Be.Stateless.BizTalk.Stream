	...
	public class EventingReadStream : Microsoft.BizTalk.Streaming.EventingReadStream
	{
		...
		public override bool CanSeek
		{
			get
			{
				...
				{
					ThrowIfDisposed();
					// do not mess with current position and corrupt fired events while stream is still being read for the 1st time
		...
		/// </returns>
		/// <remarks>
		/// The <see cref="Length"/> will be computed while the stream is being read and will consequently only be available and
		/// known accurately once the whole stream has been exhausted. While being read, <see cref="Length"/> will therefore
		/// throw a <see cref="NotSupportedException"/> exception.
		/// </remarks>
		...
		public override long Length
		{
			get
			{
				...
				{
					// avoid logging in this case as Microsoft.BizTalk.Internal.MessagePart.GetSize() systematically calls this
					// getter without first calling CanSeek getter as is expected against any Stream-derived class. see
					// Microsoft.BizTalk.Messaging, Version=3.0.1.0, Culture=neutral, PublicKeyToken=31bf3856ad364e35
					throw;
				}
				catch (Exception exception)
				{
		...
		/// </exception>
		/// <remarks>
		/// <see cref="Read"/> will always try to read as many bytes as requested to circumvent idiosyncratic clients that would
		/// fail to issue the required last extra call to <see cref="Read"/>, which would return zero (0), because that they
		...
		public override int Read(byte[] buffer, int offset, int count)
		{
			...
			try
			{
				...
				// ensures there always is a call to base Microsoft.BizTalk.Streaming.EventingReadStream.Read() that returns 0 so
				// that base EventingReadStream class behaves as expected and always fires its AfterLastReadEvent event
